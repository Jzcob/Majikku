...
public class RandomTickSpeed implements CommandExecutor {
    ...
    public RandomTickSpeed(Majikku plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (this.plugin.getConfig().getBoolean("Updated")) {
            ...
            if (sender instanceof Player && (p.hasPermission("majikku.gamerule.randomtickspeed") || p.hasPermission(Variables.gamerule))) {
                if (args.length == 0) {
                    ...
                    world.setGameRule(GameRule.RANDOM_TICK_SPEED, Integer.valueOf(a));
