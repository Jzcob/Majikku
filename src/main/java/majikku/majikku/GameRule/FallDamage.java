...
public class FallDamage implements CommandExecutor {
    ...
    public FallDamage(Majikku plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (this.plugin.getConfig().getBoolean("Updated")) {
            ...
            if (sender instanceof Player && (p.hasPermission("majikku.gamerule.falldamage") || p.hasPermission(Variables.gamerule))) {
                if (args.length == 0) {
                    ...
                    world.setGameRule(GameRule.FALL_DAMAGE, Boolean.valueOf(true));
                    ...
                    world.setGameRule(GameRule.FALL_DAMAGE, Boolean.valueOf(false));
