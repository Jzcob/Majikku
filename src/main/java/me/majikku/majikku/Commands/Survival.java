...
public class Survival implements CommandExecutor {
    ...
    Majikku plugin;
    public Survival(Majikku plugin) {this.plugin = plugin;}
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (plugin.getConfig().getBoolean("Updated")) {
            if (args.length == 0) {
                if (p.hasPermission("majikku.gamemode.survival") || p.hasPermission("majikku.gamemode.*")) {
                ...
                if (p.hasPermission("majikku.gamemode.survival.other") || p.hasPermission("majikku.gamemode.*")) {
                    Player t = Bukkit.getPlayerExact(args[0]);
                    t.setGameMode(GameMode.SURVIVAL);
                    String prefix = plugin.getConfig().getString("Settings." + "prefix");
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', prefix + "&7You have set &c" + t.getName() + "&7 to &c&lSURVIVAL&7."));
                    if (plugin.getConfig().getBoolean("Settings." + "Notify." + "notify-target")) {
                        t.sendMessage(ChatColor.translateAlternateColorCodes('&', prefix + "&7Your gamemode has been set to &c&lSURVIVAL&7."));
                    } else if (plugin.getConfig().getBoolean("Settings." + "Notify." + "notify-who-did-it")) {
                        t.sendMessage(ChatColor.translateAlternateColorCodes('&', prefix + "&c" + p.getName() + "&7 updated your gamemode to &c&lSURVIVAL&7."));
                    } else {
                        return false;
                    }
                } else {
