...
public class MaxCommandChainLength implements CommandExecutor {
    Majikku plugin;
    public MaxCommandChainLength(Majikku plugin) {this.plugin = plugin;}
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (plugin.getConfig().getBoolean("Updated")) {
            ...
            if (sender instanceof Player) {
                if (p.hasPermission("majikku.gamerule.maxcommandchainlength") || p.hasPermission(Variables.gamerule)) {
                    if (args.length == 0) {
                        ...
                        world.setGameRule(GameRule.MAX_COMMAND_CHAIN_LENGTH, a);
